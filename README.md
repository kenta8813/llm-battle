...
### ✨ 5. AIによるキャラクター設計
プロンプトを書いてClaudeに渡すと、Claudeがキャラクター設定を読んでステータスとアビリティを自分で判断して設定します。
...
- MCPサーバーはWebサーバー（`http://localhost:3000`）経由でデータにアクセスします
- Webサーバーを先に起動してからClaude Desktopを起動してください
...
このキャラクター設定を読んで、ステータスとアビリティを自分で判断して設定してください。
```
...
│       │   ├── matchmaking.js# マッチングキュー（CRUD）
│       │   └── battles.js   # バトル管理（CRUD）
